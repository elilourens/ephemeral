...
| `dm/` | direct messages: guild-less channels (`type='dm'`) + `dm_members`; `DmService.open` is idempotent per user pair |
...
`content_tsv`, `has_link`)

## Tests

- `src/test/java` — `EphemeralE2ETest` (24 tests over real HTTP + embedded PG: auth,
  roles, messages, saves, files, retention, WS, LiveKit tokens, encryption),
...
`voice_presence_snapshot` `presence_update` `presence_snapshot`
